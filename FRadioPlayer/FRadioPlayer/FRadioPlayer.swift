...
@objc public protocol FRadioPlayerDelegate: class {
     ...
     */
    @objc func radioPlayer(player: FRadioPlayer, playerStateDidChange state: FRadioPlayerState)
     ...
     */
    @objc func radioPlayer(player: FRadioPlayer, playbackStateDidChange state: FRadioPlaybackState)
     ...
     */
    @objc optional func radioPlayer(player: FRadioPlayer, itemDidChange url: URL?)
     ...
     */
    @objc optional func radioPlayer(player: FRadioPlayer, metadataDidChange artistName: String?, trackName: String?)
     ...
     */
    @objc optional func radioPlayer(player: FRadioPlayer, metadataDidChange rawValue: String?)
     ...
     */
    @objc optional func radioPlayer(player: FRadioPlayer, artworkDidChange artworkURL: URL?)
}
...
open class FRadioPlayer: NSObject {
    ...
    @objc public static let shared = FRadioPlayer()
    ...
    open private(set) var state = FRadioPlayerState.urlNotSet {
        didSet {
            ...
            delegate?.radioPlayer(player: self, playerStateDidChange: state)
        }
    }
    ...
    open private(set) var playbackState = FRadioPlaybackState.stopped {
        didSet {
            ...
            delegate?.radioPlayer(player: self, playbackStateDidChange: playbackState)
        }
    }
...
//        playerItem?.add(metadataOutput)
    ...
    private func playerItemDidChange() {
        ...
        delegate?.radioPlayer?(player: self, itemDidChange: radioURL)
    }
    ...
    private func timedMetadataDidChange(rawValue: String?) {
        ...
        delegate?.radioPlayer?(player: self, metadataDidChange: parts?.first, trackName: parts?.last)
        delegate?.radioPlayer?(player: self, metadataDidChange: rawValue)
    ...
    private func shouldGetArtwork(for rawValue: String?, _ enabled: Bool) {
        ...
        guard let rawValue = rawValue else {
            self.delegate?.radioPlayer?(player: self, artworkDidChange: nil)
            return
        }
        
        FRadioAPI.getArtwork(for: rawValue as String, size: artworkSize, completionHandler: { [unowned self] artworlURL in
            DispatchQueue.main.async {
                self.delegate?.radioPlayer?(player: self, artworkDidChange: artworlURL)
            }
        })
    }
...
//@objc extension FRadioPlayer: AVPlayerItemMetadataOutputPushDelegate {
//
//    @objc public func metadataOutput(_ output: AVPlayerItemMetadataOutput, didOutputTimedMetadataGroups groups: [AVTimedMetadataGroup], from track: AVPlayerItemTrack?) {
//
